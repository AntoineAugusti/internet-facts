...
$(document).ready(function(){
	$("#form_newsletter").submit( function(){
		...
		{
		$.post("/ajax/newsletter.php", { email: input_email },
			function(data)
			{
				if(data == 1)
				{
...
function edit_fact(id_fact) {
	...
	$(".txt_fact[data-id="+id_fact+"]").html(''
	+	'<form action="/ajax/edit_fact.php" method="post" id="edit_fact">'
	+		'<textarea name="text_fact" id="text_fact">'+txt_fact+'</textarea>'
	+		'<input type="hidden" name="id_fact" value="'+id_fact+'"/>'
	+		'<br /><br />'
	+		'<input type="submit" value="Edit this fact"/><br/>'
	+		'<div class="clear"></div>'
	+	'</form>');
