...
$(document).ready(function(){
	$("#submit_fact_admin").submit( function(){
		...
		textarea_text_fact = $('#textarea_text_fact').val();
		...
		$.post("/ajax/add_fact.php", { text_fact: textarea_text_fact, admin: true },
			function(data)
			{
				...
				$('#notification').html(div_response).delay(2000).slideUp();
...
$(document).ready(function(){
	$("#submit_fact_user").submit( function(){
		$('#notification').html('');
		$('#notification').html('<div class="error">Loading..</div>').slideDown();
		username_form = $('#username').val().toLowerCase();
		email_form = $('#email').val();
		text_fact_form = $('#textarea_text_fact').val();

		if (validateUsername(username_form) == false || username_form.length === 0)
		{
			$('#notification').html('<div class="error">Please enter a valid username.</div>').delay(2000).slideUp();
			$('#username').val('').focus();
		}
		else 
		{
			...
			{
				$('#notification').html('<div class="error">Please enter a valid email address.</div>').delay(2000).slideUp();
				$('#email').val('').focus();
			}
			else
			{
				...
				{
					$.post("/ajax/add_fact.php", { text_fact: text_fact_form, username: username_form, email: email_form, admin: false },
					function(data)
					{
						if(data == 1)
						{
